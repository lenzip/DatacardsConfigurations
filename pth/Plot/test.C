...
void Plot_AM_HWidth_Propaganda() {
...
//  int which = 1;
 ...
 TString nameChannel;
 if   (which == 0)        { nameChannel = Form ("of_pth1/"); }       //---- signal injection
 else if (which == 1)     { nameChannel = Form ("of_pth1/"); }
...
//  TString cutNameBefore = Form("sig/%shisto_",nameChannel.Data());
 ...
 if      (which == 0)   { f[0] = new TFile("../mll-mth_8TeV_pth1/postFit/Hwidth-pth1-of-error-signalInjection.root");  doSignalInjection = true; }
 else if (which == 1)   { f[0] = new TFile("../mll-mth_8TeV_pth1/postFit/Hwidth-pth1-of-error-data.root");  doSignalInjection = false; }
...
//  hs->addWeight(NY-minNY); //---- add S/(S+B) weight ---> used only for propaganda plot and data-background
 ...
 //double vedges[] = {12,30,45,60,75,100,125,150,175,200} ; //mll

 double vedges[] = {60,70,80,90,100,110,120,140,160,180,200,220,240,260,280};  // mT
...
//  double vedges[] = {0, 20, 50, 75, 100, 150, 200, 250, 300};
 ...
 c1bis->Print("bkgSub_mT.pdf");
 c1bis->Print("bkgSub_mT.png");
 c1bis->Print("bkgSub_mT.eps");
