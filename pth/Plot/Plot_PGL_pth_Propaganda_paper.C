...
void Plot_PGL_pth_Propaganda_paper(int channel, int what) {
...
//  TString cutNameBefore = Form("sig/%shisto_",nameChannel.Data());
 ...
 //TString cutNameBefore = Form("sig/of_pthincl/histo_");
 TString cutNameBefore = Form("init/of_pthincl/histo_");
 //TString cutNameBefore = Form("histo_");
 ...
 if (which == 0)
    {
      f[0] =
        ...
        TFile (Form
               ...
               ("../mll-mth_8TeV_pthincl3/postFit/Hwidth-pthincl-of-error-signalInjection.root"));
  ...
  else if (which == 1)
    {
      f[0] =
        new
        TFile (Form
               ("../mll-mth_8TeV_pthincl/postFit/Hwidth-pthincl-of-error-data.root"));
...
//  hs->setLabel("mll*");
 ...
 hs->setLumi(19.4);
 int ptHBins[7] = {0, 15, 45, 85, 125, 165, 200};
 TString bigLabel;
 if (channel < 5) 
  bigLabel=Form("p_{T}^{H} [%d, %d) GeV", ptHBins[channel], ptHBins[channel+1]);
 else 
  bigLabel=Form("p_{T}^{H}#geq %d GeV", ptHBins[channel]);
 //hs->addLabel("#splitline{     p_{T_{H}}}{     #sqrt{s} = 8 TeV}");
 hs->addLabel(("#splitline{     #sqrt{s} = 8 TeV}{\n"+bigLabel+"}").Data());
//  
 TString name;
...
//  int NMAXY = 6;  
 ...
 int NMAXX = 13;  //---- variable bin
 int NMAXY = 9;  

 //if (string(channel) == "pth5" || string(channel) == "pth6")
 // NMAXX = 11;

 //---- all ----
 int minNY = 0;
 int minNX = 13*channel;
...
//  int NY = NMAXY;
  ...
  */
  int maxi = (what == 0) ? NY : NX;
  int maxj = (what == 0) ? NX : NY;
  int mini = (what == 0) ? minNY : minNX;
  int minj = (what == 0) ? minNX : minNY;
  for (int i=mini; i<maxi; i++){
   binsToSelect.clear();
   for (int j=minj; j<maxj; j++){
    int bin = (what == 0) ? j*maxi+i+1 : i*maxj +j +1;
    binsToSelect.push_back (bin);
...
//    std::cout << "I'm here: " << WHEREAMI << std::endl; WHEREAMI++;
   ...
   name = Form("%sTop0jet%s",cutNameBefore.Data(),cutNameAfter.Data());
   vectTHBkg.push_back ( FilterBins(binsToSelect, (TH1F*) f[iFile]->Get(name)) );
   ...
   name = Form("%sTopge1jetBin%d%s",cutNameBefore.Data(), channel, cutNameAfter.Data());
   vectTHBkg.push_back ( FilterBins(binsToSelect, (TH1F*) f[iFile]->Get(name)) );
   vectNameBkg.push_back ("Top");
   vectColourBkg.push_back(400);
   vectSystBkg.push_back(0.10);
...
//    }
 ...
 if (doSignalInjection){
  ...
  for (int iBkg = 1; iBkg < vectTHBkg.size(); iBkg++) {
    //tempData->Add((TH1F*) vectTHBkg.at(iBkg)->Clone());
    hs->setDataHist ((TH1F*) vectTHBkg.at(iBkg));
  }
  for (int iSig = 0; iSig < vectTHSig.size(); iSig++) {
    //tempData->Add((TH1F*) vectTHSig.at(iSig)->Clone());
    hs->setDataHist ((TH1F*)vectTHSig.at(iSig));
  }
  std::cout << "fake data set!!" << std::endl;
 }
 */
...
//  double vedges[] = {0, 20, 50, 75, 100, 150, 200, 250, 300};
  ...
  double
    vedges_mt[] =
    { 60, 70, 80, 90, 100, 110, 120, 140, 160, 180, 200, 220, 240, 280 };
  double vedges_mt_2[] = {60,80,100,120,140,160,180,200,220,240,260,280};  
...
//   }
 ...
 std::stringstream namefile;
  if (what == 0){
    namefile << "mTBin" << channel; 
  }  
  else {
      namefile <<  "mllBin" << channel;
    }

  c1->Print (Form ("../mll-mth_8TeV_pthincl3/plots/%s.pdf", namefile.str().c_str()));
  c1->Print (Form ("../mll-mth_8TeV_pthincl3/plots/%s.png", namefile.str().c_str()));
  c1->Print (Form ("../mll-mth_8TeV_pthincl3/plots/%s.eps", namefile.str().c_str()));
  c1->Print (Form ("../mll-mth_8TeV_pthincl3/plots/%s.C", namefile.str().c_str()));

  c1bis->Print (Form ("../mll-mth_8TeV_pthincl3/plots/bkgSub_%s.pdf", namefile.str().c_str()));
  c1bis->Print (Form ("../mll-mth_8TeV_pthincl3/plots/bkgSub_%s.png", namefile.str().c_str()));
  c1bis->Print (Form ("../mll-mth_8TeV_pthincl3/plots/bkgSub_%s.eps", namefile.str().c_str()));  
  c1bis->Print (Form ("../mll-mth_8TeV_pthincl3/plots/bkgSub_%s.C", namefile.str().c_str()));  
