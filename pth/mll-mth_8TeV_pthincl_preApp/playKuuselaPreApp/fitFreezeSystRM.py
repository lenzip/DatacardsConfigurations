...
command = "combine --saveWorkspace --expectSignal=1 -t -1 -M MultiDimFit hww-19.47fb.mH125.of_pthincl_shape_workspace.root --algo=singles --saveToys&> logFull.txt"
os.popen(command)
os.popen("mv higgsCombineTest.MultiDimFit.mH120.123456.root full.root")
os.popen("mv mlfit.root mlfit_full.root")
#sys.exit(0)
command = "combine --saveWorkspace --expectSignal=1 -t -1 -M MultiDimFit hww-19.47fb.mH125.of_pthincl_shape_workspace.root --algo=singles --freezeNuisances="+",".join(changing_RM)+" &> logNotchangingRM.txt"
os.popen(command)
os.popen("mv higgsCombineTest.MultiDimFit.mH120.root stat.root")
os.popen("mv mlfit.root mlfit_notchangingRM.root")

command = "combine --saveWorkspace --expectSignal=1 -t -1 -M MultiDimFit hww-19.47fb.mH125.of_pthincl_shape_workspace.root --algo=singles --freezeNuisances="+",".join(allsyst)+" &> logStatOnly.txt"
os.popen(command)
os.popen("mv higgsCombineTest.MultiDimFit.mH120.root stat.root")
...
for syst in changing_RM:
  ...
  command = "combine -t -1 -M MultiDimFit hww-19.47fb.mH125.of_pthincl_shape_workspace.root --algo=singles --toysFile full.root --setPhysicsModelParameters "+syst+"="+str(error)+" --freezeNuisances="+syst+" &> log"+syst+"_Up.txt"
  os.popen(command)
