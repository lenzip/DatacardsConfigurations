...
def makePlot(central, r, stat, name, syst=[]):
  ...
  for i in range(1, 7):
    print hfake.GetBinContent(i)
    h.SetBinContent(i, rcopy[i-1]*(central[i-1]-hfake.GetBinContent(i)))
    h.SetBinError(i, stat[i-1]*(central[i-1]-hfake.GetBinContent(i)) )
...
for syst in correlated:
  ...
  for i in range(len(errTotUp)):
    ...
    print "syst:", syst, " ", rSystDown[i]-r[i]
